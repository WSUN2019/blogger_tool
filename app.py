...
_feed_cache = None
_active_feed = None  # filename only (not full path)


def _list_feed_files():
    """Return sorted list of .atom filenames in blog_backup/ (not in subdirs)."""
...
def _resolve_active_feed():
    """Return active filename, auto-selecting the most recent if needed."""
    global _active_feed
...
def feed_generate():
    ...
    for p in all_posts:
        ...
        status    = p.get('status', 'LIVE')
        labels    = p.get('labels', [])
        filename  = urlparse(p.get('url', '')).path or ''
        lines += [
            "  <entry>",
        ...
        if filename:
            lines.append(f"    <blogger:filename>{filename}</blogger:filename>")
        lines.append("  </entry>")
    lines.append("</feed>")
...
@app.route('/feed/select', methods=['POST'])
@login_required
def feed_select():
    global _feed_cache, _active_feed
    data = request.get_json() or {}
